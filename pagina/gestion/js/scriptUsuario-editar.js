    ...
    html = `
      <div class="row gx-4 gx-lg-5 align-items-center" id="mostrarUsuario">
        ...
        <p id="error-usuario-apellidos" class="text-danger"></p>
          ...
          <button type="button" onclick="subirCurriculum()" style="background-color: #009452; border-color: #009452;" class="btn btn-primary">Subir</button>
        </div>
          <button type="submit" class="btn custom-btn-color" onclick="actualizarDatos()" id="guardarDatos">Guardar</button>
        </div>
      </div>
...
const url = `http://127.0.0.1:8000/api/usuario/${idUsuario}`;
const metodo = 'GET';
...
function actualizarDatos() {
  ...
  if (contarDigitos(telefono) != 9 || !isNumber(telefono)) {
  ...
  fetch(urlActualizar, {
    ...
    .then(data => {
      ...
      window.location.replace(`perfil.html?id=${idUsuario}`);
...
function subirCurriculum() {
  const url = `http://localhost:8000/api/subir-curriculum/${idUsuario}`;
